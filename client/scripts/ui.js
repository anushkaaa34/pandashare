...
class PeerUI {
  ...
  _onFilesSelected(e) {
    ...
    console.log("Checking for peer id", this._peer.id);
    // reset input
    $input.value = null;
  }
...
Events.on("load", () => {
  ...
  function drawCircle(radius) {
    ctx.beginPath();
    let color1 = Math.round(
      255 - (((255 - 255) * (radius / Math.min(w, h))) ^ 1)
    );
    let color2 = Math.round(
      255 - (((255 - 128) * (radius / Math.min(w, h))) ^ 1)
    );
    let color3 = Math.round(
      255 - (((255 - 0) * (radius / Math.min(w, h))) ^ 1)
    );
